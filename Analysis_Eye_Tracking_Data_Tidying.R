...
EQ.data <- read.csv("EQ_Data_AQ_Data.csv")  
...
#### GE Preprocessing raw data for all participants ####
...
#Coding trials that ignores whether it's scrambled or not, to allow identification of sociality later
...
#### summarising GE data for each participant ####
summary.ge<-data.frame(pps=unique(ge.raw$PP))

# loop through participants to count time excluded trials (<70ms or >500ms)
...
# only keep trials in which the first saccade was between 70-500ms from onset in analysis
## this needs to be here to allow counting of trials failing this criterion before
...
#Only keep participants with at least 30 valid scrambled and 30 valid unscrambled trials
...
#### FV raw data preprocessing ####
#splitting by scrambled vs. unscrambled 
...
#### Summarising FV data for each participant ####
...
summary.EQ<-subset(EQ.data,select = c(pps,EQ.Total,AQ,Social.skill,Attention.switching,Communication,Imagination,Attention.to.detail))
...
## AQ analyses
cor.test(summary.ge.eq$Social.skill,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Social.skill,summary.ge.eq$angle.scr)

cor.test(summary.ge.eq$Attention.switching,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Attention.switching,summary.ge.eq$angle.scr)

cor.test(summary.ge.eq$Communication,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Communication,summary.ge.eq$angle.scr)

cor.test(summary.ge.eq$Communication,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Communication,summary.ge.eq$angle.scr)

cor.test(summary.ge.eq$Imagination,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Imagination,summary.ge.eq$angle.scr)

cor.test(summary.ge.eq$Attention.to.detail,summary.ge.eq$angle.unsc)
cor.test(summary.ge.eq$Attention.to.detail,summary.ge.eq$angle.scr)
...
## AQ analyses
cor.test(summary.fv.eq$AQ,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$AQ,summary.fv.eq$scr.prop)

cor.test(summary.fv.eq$Social.skill,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Social.skill,summary.fv.eq$scr.prop)


cor.test(summary.fv.eq$Social.skill,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Social.skill,summary.fv.eq$scr.prop)

cor.test(summary.fv.eq$Attention.switching,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Attention.switching,summary.fv.eq$scr.prop)

cor.test(summary.fv.eq$Communication,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Communication,summary.fv.eq$scr.prop)

cor.test(summary.fv.eq$Imagination,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Imagination,summary.fv.eq$scr.prop)

cor.test(summary.fv.eq$Attention.to.detail,summary.fv.eq$unsc.prop)
cor.test(summary.fv.eq$Attention.to.detail,summary.fv.eq$scr.prop)




## testing claim that EQ correlates with social component of AQ
cor.test(summary.fv.eq$Social.skill,summary.fv.eq$EQ.Total) ## not a strong effect
